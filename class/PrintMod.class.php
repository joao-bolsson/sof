...
final class PrintMod {
    ...
    public static function getRelatorioFaturamento(int $id, int $producao, int $financiamento, int $complexidade, int $comp, int $ano, string $dataI, string $dataF): string {
        ...
        $return .= "
            <fieldset class=\"preg\">
                    ...
                    <h6>Produção: " . ARRAY_PRODUCAO[$producao] . "</h6>
                    <h6>Financiamento: " . ARRAY_FINANCIAMENTO[$financiamento] . "</h6>
                    <h6>Complexidade: " . ARRAY_COMPLEXIDADE[$complexidade] . "</h6>
                    <h6>Competência: " . $mes . "</h6>
                    <h6>Ano: " . $ano . "</h6>
            </fieldset><br>";

        $query = Query::getInstance()->exe("SELECT DATE_FORMAT(lancamento, '%d/%m/%Y') AS lancamento, valor FROM faturamento WHERE producao = " . $producao . " AND financiamento = " . $financiamento . " AND complexidade = " . $complexidade . " AND competencia = " . $comp . " AND ano = " . $ano . " AND (faturamento.lancamento BETWEEN '" . $dI . "' AND '" . $dF . "') AND id_contr = " . $id);

        $return .= "<fieldset>";
        $table = new Table('', 'prod', ['Lançamento', 'Valor'], true);

        $totVar = 0;

        $isToSum = false;

        //if (producao == "SIA" && $financiamento == "MAC" && $complexidade == "MC") {
        if ($producao == 1 && $financiamento == 2 && $complexidade == 1) {
            // nao soma
        } else if ($obj->producao == 2 && $financiamento == 2 && $complexidade == 1) {
            // $producao == "SIH" && $financiamento == "MAC" && $complexidade == "MC") {
            // nao soma
        } else {
            $isToSum = true;
        }
